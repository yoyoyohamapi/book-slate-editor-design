...
同理，B 客户端受到 ACK 以后也更新自己的版本为 1，紧接着到来的 A0'(1) 也可直接应用，更新版本到2。由于服务端当前版本也是 2，因此 A1(2) 可以直接落库，服务端更新版本为 3。并广播 A1(2) 到其它客户端，以及向客户端 A 回调 A1 的 ACK：
<p align="center">
...
这个架构中，基于 ACK 机制，不但限制了 OP 提交的频度，也方便地通过简单地版本号就表示了文档上下文，避免维护一个 N 维状态向量。它非常接近实际应用了。但本文的描述也只是实际工程的冰山一角，一个稳定协同过程还面临着诸多问题：
